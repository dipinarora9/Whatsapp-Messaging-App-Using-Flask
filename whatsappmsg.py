...
def send_message(num, mes):
    message = client.messages.create(from_='whatsapp:#add twilio number here', body='%s' % mes,
                                     to='whatsapp:+91%d' % num)
    return message


def read_message(num):
    # message = client.messages.list(date_sent=(now.strftime("%Y, %m, %d, %H, %M")), from_='whatsapp:+91%d' % num,
    #                                to='whatsapp:#add twilio number here')
    message = client.messages.list(date_sent=datetime, )
